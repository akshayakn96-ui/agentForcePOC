    ...
    private fun configureServiceAgent(config: ReadableMap, promise: Promise) {
        ...
        scope.launch(Dispatchers.Main) {
            ...
            try {
                ...
                // The SDK only exposes header actions through TopAppBarBuilder. Use the
                // bridge builder for Service Agents too, so its extra header icon is
                // available in the Service conversation UI. The developer name is the
                // stable title before the SDK has resolved an agent label.
                agentforceConfigBuilder.setTopAppBarBuilder(
                    BridgeTopAppBarBuilder(serviceConfig.esDeveloperName)
                )

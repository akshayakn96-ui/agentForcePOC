...
class AgentforceService {
  ...
  setUIDelegate(delegate: UIDelegate): void {
    console.log('[AgentforceService] setUIDelegate called');
    console.log('[AgentforceService] Delegate:', delegate);

    this.uiDelegate = delegate;
    this.setupUIDelegateListeners();

    console.log('[AgentforceService] Event Emitter:', this.eventEmitter);

    console.log(
      '[AgentforceService] enableUIDelegateForwarding available:',
      !!AgentforceModule?.enableUIDelegateForwarding,
    );
  ...
  async startNewConversation(): Promise<boolean> {
    ...
    try {
      ...
      console.log('[AgentforceService] New conversation started successfully', result);
      return result?.success ?? true;
    } catch (error) {
